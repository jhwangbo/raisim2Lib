...
.. code-block:: bash
   ...
   <raisim-install>/bin/rayrai_raisim_tcp_viewer --fullscreen --minimize-panels
...
``--window-size``, ``--fullscreen``, ``--minimize-panels``, ``--auto-frame``,
``--camera-lookat``, ``--camera-offset``, ``--force-camera-lookat``,
``--screenshot``, ``--screenshot-dir``, ``--record-session``,
``--replay-session``, ``--replay-speed``, ``--replay-loop``,
``--export-scene``, ``--trajectory-csv``, ``--server-list``,
``--wait-for-server``, ``--exit-after``, and ``--help``.
...
- Provides UI controls for host/port connection, recent endpoints, resource
  search directories, collision body display, contacts, camera speed, lighting,
  render quality, UI scale, and world-frame display.
- Provides batch-friendly startup framing, screenshots, PNG output directory
  ...
  and scene/object JSON diagnostics.
