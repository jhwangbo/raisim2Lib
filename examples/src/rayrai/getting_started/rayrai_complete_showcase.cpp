...
int main(int argc, char* argv[]) {
  ...
  ExampleApp app;
  if (!app.init("rayrai_complete_showcase", 1280, 720)) {
    return -1;
  }
  ...
  hm->setAppearance("0.55,0.55,0.55,1.0");
  ...
  while (!app.quit) {
    ...
    // --- Path 1: rayrai image capture (default) ---
    // This path renders the sensor cameras with rayrai. It gives RGB and
    // rendered depth, including visual geometry, materials, and lighting.
    // To use the CPU-only depth capture path below, comment this rayrai block
    // and uncomment the CPU depth camera block.
    ...
    // --- Path 2: CPU depth camera capture ---
    // Uncomment this block after rebuilding RaiSim with
    // World::captureDepthCamera. This path does not use rayrai. It casts CPU
    // rays against collision geometry and returns depth, object segmentation ids,
    // optional world-frame hit points, and a timestamp.
#if 0
    depthCam->updatePose();
    raisim::World::DepthCameraProperties cpuProp;
    cpuProp.width = std::max(1, std::min(64, depthCam->getProperties().width));
    cpuProp.height = std::max(1, std::min(48, depthCam->getProperties().height));
    ...
    RSINFO("cpu depth camera t=" << cpuFrame.timeStamp
           ...
           << " center_object=" << cpuFrame.segmentation[cpuCenter])
#endif
